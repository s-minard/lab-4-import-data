...
#| label: load-data
#| eval: false
buoy_2024 <- read_table("data/data-raw/buoy_2024.txt")
#Note to my future self: I made the .txt file by copy/pasting the text found at this link (https://jcanner.github.io/stat_210_2025_website/docs/labs/student/data/buoy_2024.txt) into the TextEdit app
```
...
#| eval: false
buoy_2024 <- read_table("data/data-raw/buoy_2024.txt", comment = "#")
...
#| eval: false
write_csv(buoy_2024, file = "data/data-raw/buoy_update.csv")
```
...
#| eval: false
buoy_update <- read_csv("data/data-raw/buoy_update.csv", na = c("99",))
```
...
#The provided code resulted in an error
buoy_final <- buoy_update |> 
  mutate(across(.cols = everything(), ~parse_double(.x))) 

#Typed ?across into the console and experimented based on the examples until I got it to do what I wanted (make the columns dbl)
buoy_final <- buoy_update |> 
  mutate(across(.cols = everything(), ~as.double(.x)))

buoy_final
write_csv(buoy_final, file = "data/data-clean/buoy_final.csv")
...
```{r}
#| label: load-buoy-final
buoy_final <- read_csv("data/data-clean/buoy_final.csv")
```
...
ggplot(data = buoy_final, 
       aes(x = mm,
           y = atmp)) +
  geom_jitter(aes(color = factor(mm)), width = 0.1, alpha = 0.2) +
  geom_smooth(color = "black") +
  scale_color_viridis_d() +
  labs(x = "Month in 2024",
       y = "Air Temperature (Celsius)",
       caption = "Source: NOAA National Buoy Center",
...
>Corporate Pride Accountability from TinyTuesday: https://github.com/rfordatascience/tidytuesday/tree/main/data/2022/2022-06-07
The dataset contains information about corporations that make donations to LGBTQ Pride parades or organizations to appear progressive, but also donate to anti-LGBTQ politicians. Contains the "political giving of specific Fortune 500 companies to anti-LGBTQ+ politicians in six states." 

> Questions: 1. Which corporations have donated the most money to anti-LGBTQ politicians? What is the geographical spread of their donations? 2. What is the typical ratio of money given to Pride organizations versus anti-LGBTQ politicians? 3. Is there a correlation between the level of Pride event sponsorship and donations to anti-LGBTQ+ politicians? 4. How have the donations changed over time? 5. Which anti-LGBTQ politicians receive the most corporate funding? 
